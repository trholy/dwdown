...
class TestSessionHandler(unittest.TestCase):
    ...
    @patch('dwdown.utils.network_handling.Retry')
    @patch('dwdown.utils.network_handling.HTTPAdapter')
    @patch('dwdown.utils.network_handling.Session')
    def test_create_session(self, mock_session, mock_http_adapter, mock_retry):
        ...
        mock_http_adapter.assert_called_with(max_retries=mock_retry_instance)
        # Session() is called
        mock_session.assert_called()
        # session.mount() is called
        mock_session_instance.mount.assert_called_with("https://", mock_http_adapter_instance)
        
        # Verify that handler._session is the mock session
        self.assertEqual(handler.get_session(), mock_session_instance)

    @patch('dwdown.utils.network_handling.Session', side_effect=Exception("Failed to create session"))
    def test_create_session_exception(self, mock_session):
        # Test the _create_session method when an exception occurs
        # Since _create_session is called in __init__, instantiation raises the error
